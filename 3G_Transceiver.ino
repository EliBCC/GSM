...
void setup() {
  ...
  OCR1A=65535; //Set frequency to 0.24HZ (Clock Frequency/Prescaler/Desired Frequency=OCR1A)
  
  TCCR1B|=(1<<WGM12); //CTC Mode (reset timer on overflow)
  
  TCCR1B|=(1<<CS12);  //Set prescaler to 1024
  TCCR1B|=(1<<CS10);

  TIMSK1|=(1<<OCIE1A); //Enable ISR call on timer overflow
   ...
   { // send an SMS!
        ...
        while(loop)
        {
          ...
          else if(message[0]=='2')
          {
            ...
            char character1=144;
            char character2=65;
            str=/*"☺,"+*/(String)(character1)+","+(String)(character2)+","+(String)(LoadVoltage)+","+(String)(LoadCurrent)+","+(String)(Power)+","+(String)(AtmTemp)+","+(String)(SolTemp)+","+(String)(WaterBreakerFlag);
...
/*
 * ISR should interrupt normal operation (not yet implemented) to check if an SMS has been recieved, and if it has, print the SMS
 */
ISR(TIMER1_COMPA_vect)  //Interrupt Service Routine
{
  //Takes 450ms to check for messages if no new message
  //Takes 1500ms to print out a new message
    check_get_sms();
